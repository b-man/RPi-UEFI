...
BUILD_ROOT=$WORKSPACE/Build/Pi3Board/"$TARGET"_"$TARGET_TOOLS"
...
if [[ $TARGET == RELEASE ]]; then
  build -p $WORKSPACE/RaspberryPiPkg/Pi3BoardPkg/Pi3BoardPkg.dsc -a ARM -t $TARGET_TOOLS -b $TARGET -D DEBUG_TARGET=RELEASE ${2:-} ${3:-} ${4:-} ${5:-} ${6:-} ${7:-} ${8:-}
else
  build -p ${WORKSPACE:-}/RaspberryPiPkg/Pi3BoardPkg/Pi3BoardPkg.dsc -a ARM -t $TARGET_TOOLS -b $TARGET ${1:-} ${2:-} ${3:-} ${4:-} ${5:-} ${6:-} ${7:-} ${8:-}
fi
...
if [ $retval -eq 0 ]; then
  ...
  #
  cat $TMP_DIR/startup.bin $TMP_DIR/pad.bin $BUILD_ROOT/FV/PI3BOARD_EFI.fd > $BUILD_ROOT/$BOOT_IMAGE
  ...
  echo -e "Pi3Boot Image Build ## Succeeded ##\n"
else
  echo -e "Pi3Boot Image Build ## Failed ##\n"
fi
